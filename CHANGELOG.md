...
- **Static sites are their own resource, not a kind of application:** they host prebuilt HTML, CSS and JavaScript from object storage, and they are unreachable through `/api/v1/apps` — the endpoints live under `/api/v1/static`. There is no compute resource, no replicas, no ports, no environment variables and no cost. Two rules trip up a first attempt and are called out up front: the name is exact and globally unique, so a taken name is a `422` rather than a silent rename, and the content source is fixed at creation because it decides what gets provisioned. Four sources get a runbook each — `github` and `git_push` are built for you with the generator auto-detected across 33+ frameworks, while `zip` and `sftp` take already-built output, so uploading a project rather than its output directory publishes the source files
